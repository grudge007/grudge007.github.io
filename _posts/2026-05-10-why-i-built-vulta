...
So I added hash tracking to detect modified files and upload only the changed ones. That made pushes much faster and reduced unnecessary transfers between systems.
I had already added support for `.vultaignore` files so directories like `node_modules`, `__pycache__`, and other unnecessary files could be skipped.
